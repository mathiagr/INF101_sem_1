...
import inf101.v17.boulderdash.bdobjects.AIBDPlayer;
...
public class BDMap {
	...
	/**
	 * Introducing a Map (Hashmap) for making getPosition method run in constant time.
	 * We still use the grid to extract objects using position
	 */
	 
	public Map<IBDObject, Position> hashmap;
	...
	public BDMap(IGrid<Character> map) {
		...
		//Chose between "AI" player or full control
		this.player = new BDPlayer(this);
		//this.player = new AIBDPlayer(this); //Using AI Player
		
		//Hashmap for for storing Objects and Positions 
		hashmap = new HashMap<IBDObject, Position>(map.getWidth()*map.getHeight());
		fillGrid(map);
	}
	...
	/**
	 * True if an object can move to position (x, y), false otherwise.
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	public boolean canGo(int x, int y) {
		...
		IBDObject obj = get(pos);
		
		if (obj instanceof BDWall) {
			return false;
		}
		return true;
	}
	...
	private void fillGrid(IGrid<Character> inputmap) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				//Filling hashmap with grid objects
				Position pos = new Position(x,y);
				hashmap.put(obj, pos);
			}
		}
	...
	//Slow method going through all grid points.
		/*
		...
		*/
		...
		//Fast method, constant time searching in Hashmap to retrieve position of object
	...
	public void set(int x, int y, IBDObject element) {
		...
		Position pos = new Position(x,y);
		hashmap.put(element, pos); //Setting hashmap keys and objects
	}

	/**
	 * Step method will perform a step (iteration) on every object in every postions
	 * in the grid each time it's called.
	 */
	public void step() {
		for(int x = 0; x<grid.getWidth(); x++){
			for(int y = 0; y<grid.getHeight(); y++){
